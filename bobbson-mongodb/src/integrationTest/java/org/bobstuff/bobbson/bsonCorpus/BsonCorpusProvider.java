              ...
              Arguments.of(
                  Named.named(
                      file.getName()
                          ...
                          + ":"
                          + corpus.getValid().get(i).getDescription(),
                      corpus),
                  i,
                  BsonCorpus.BsonCorpusTestCaseType.VALID));
        }
      }
      if (corpus.getDecodeErrors() != null) {
        for (var i = 0; i < corpus.getDecodeErrors().size(); ++i) {
          arguments.add(
              Arguments.of(
                  Named.named(
                      file.getName()
                          ...
                          + corpus.getDescription()
                          + ":"
                          + corpus.getDecodeErrors().get(i).getDescription(),
                      corpus),
                  i,
                  BsonCorpus.BsonCorpusTestCaseType.DECODE_ERROR));
        }
      }
    }
