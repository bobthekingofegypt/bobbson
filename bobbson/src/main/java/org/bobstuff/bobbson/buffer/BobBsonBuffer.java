...
public interface BobBsonBuffer {
  /**
   * Return the underlying array of the buffer. Providing raw access can allow for performance
   * improvements on writes by avoiding repeated calls to write individual bytes.
   *
   * <p>Direct changes to the byte array will effect the buffer but will not move its head or tail,
   * user is required to keep those values in sync with any changes they make
   *
   * <p>If unsure buffer can provide access to array call {@code hasArray} first.
   *
   * @return raw byte array or null
   * @throws UnsupportedOperationException if buffer does not support providing array
   */
  byte[] getArray();
  ...
  /**
   * Move the tail of the buffer forward by {@code size} bytes so next write operation happens at
   * tail + size location
   *
  ...
  /**
   * Sets the current head position to given position, this is the location where the next read will
   * happen
   *
  ...
  /**
   * Sets the current tail position to give position, this is the location where the next write will
   * happen
   *
  ...
  /**
   * Gets the current limit on the buffer. This is the maximum value that tail can be set to and
   * indicates the maximum length of the buffer
   *
  ...
  /**
   * Comparator for byte ranges read in through {@code BobBsonBuffer.readUntil}. Used to avoid
   * decoding UTF8 strings where raw comparisons are possible
   */
  interface ByteRangeComparator {
    /**
     * Gets the current weak hash of the bytes available in the comparator.
     *
     * <p>The weak hash is the sum of all byte values present in the range
     *
    ...
    /**
     * Compares the given hash value to the comparators hash and if they match compares the raw
     * arrays, if they don't returns false straight away.
     *
     * @param value byte array to compare against
