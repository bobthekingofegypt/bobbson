...
/**
 * A binary integer decimal representation of a 128-bit decimal value, supporting 34 decimal digits
 * of significand and an exponent range of -6143 to +6144.
 *
 * @since 3.4
 * @see <a
 *     href="https://github.com/mongodb/specifications/blob/master/source/bson-decimal128/decimal128.rst">BSON
 *     Decimal128 specification</a>
 * @see <a href="https://en.wikipedia.org/wiki/Binary_Integer_Decimal">binary integer decimal</a>
 * @see <a href="https://en.wikipedia.org/wiki/Decimal128_floating-point_format">decimal128
 *     floating-point format</a>
 * @see <a href="http://ieeexplore.ieee.org/document/4610935/">754-2008 - IEEE Standard for
...
public final class Decimal128 extends Number implements Comparable<Decimal128> {
  ...
  private static final Set<String> NEGATIVE_NaN_STRINGS =
      new HashSet<String>(singletonList("-nan"));
  private static final Set<String> POSITIVE_INFINITY_STRINGS =
      new HashSet<String>(asList("inf", "+inf", "infinity", "+infinity"));
  private static final Set<String> NEGATIVE_INFINITY_STRINGS =
      new HashSet<String>(asList("-inf", "-infinity"));

  /**
   * A constant holding the positive infinity of type {@code Decimal128}. It is equal to the value
   * return by {@code Decimal128.valueOf("Infinity")}.
   */
  public static final Decimal128 POSITIVE_INFINITY = fromIEEE754BIDEncoding(INFINITY_MASK, 0);

  /**
   * A constant holding the negative infinity of type {@code Decimal128}. It is equal to the value
   * return by {@code Decimal128.valueOf("-Infinity")}.
   */
  public static final Decimal128 NEGATIVE_INFINITY =
      fromIEEE754BIDEncoding(INFINITY_MASK | SIGN_BIT_MASK, 0);

  /**
   * A constant holding a negative Not-a-Number (-NaN) value of type {@code Decimal128}. It is equal
   * to the value return by {@code Decimal128.valueOf("-NaN")}.
   */
  public static final Decimal128 NEGATIVE_NaN = fromIEEE754BIDEncoding(NaN_MASK | SIGN_BIT_MASK, 0);

  /**
   * A constant holding a Not-a-Number (NaN) value of type {@code Decimal128}. It is equal to the
   * value return by {@code Decimal128.valueOf("NaN")}.
   */
  public static final Decimal128 NaN = fromIEEE754BIDEncoding(NaN_MASK, 0);

  /**
   * A constant holding a postive zero value of type {@code Decimal128}. It is equal to the value
   * return by {@code Decimal128.valueOf("0")}.
   */
  public static final Decimal128 POSITIVE_ZERO =
      fromIEEE754BIDEncoding(0x3040000000000000L, 0x0000000000000000L);

  /**
   * A constant holding a negative zero value of type {@code Decimal128}. It is equal to the value
   * return by {@code Decimal128.valueOf("-0")}.
   */
  public static final Decimal128 NEGATIVE_ZERO =
      fromIEEE754BIDEncoding(0xb040000000000000L, 0x0000000000000000L);
   ...
   * @see <a
   *     href="https://github.com/mongodb/specifications/blob/master/source/bson-decimal128/decimal128.rst#from-string-representation">
  ...
  /**
   * Create an instance with the given high and low order bits representing this Decimal128 as an
   * IEEE 754-2008 128-bit decimal floating point using the BID encoding scheme.
   *
  ...
  private Decimal128(final BigDecimal initialValue, final boolean isNegative) {
    ...
    if (value.unscaledValue().bitLength() > MAX_BIT_LENGTH) {
      throw new AssertionError(
          "Unscaled roundedValue is out of range for Decimal128 encoding:" + value.unscaledValue());
    }
  ...
  private BigDecimal clampAndRound(final BigDecimal initialValue) {
    ...
    if (-initialValue.scale() > MAX_EXPONENT) {
      ...
      // Only round exactly (by removing precision that is all zeroes).  An exception is thrown if
      // the rounding would be inexact:
  ...
  /**
   * Gets the high-order 64 bits of the IEEE 754-2008 128-bit decimal floating point encoding for
   * this Decimal128, using the BID encoding scheme.
   *
  ...
  /**
   * Gets the low-order 64 bits of the IEEE 754-2008 128-bit decimal floating point encoding for
   * this Decimal128, using the BID encoding scheme.
   *
   ...
   * @throws ArithmeticException if the Decimal128 value is NaN, Infinity, -Infinity, or -0, none of
   *     which can be represented as a BigDecimal
   */
  ...
  // Make sure that the argument comes from a call to bigDecimalValueNoNegativeZeroCheck on this
  // instance
  ...
  /**
   * Converts this {@code Decimal128} to a {@code int}. This conversion is analogous to the
   * <i>narrowing primitive conversion</i> from {@code double} to {@code int} as defined in
   * <cite>The Java&trade; Language Specification</cite>: any fractional part of this {@code
   * Decimal128} will be discarded, and if the resulting integral value is too big to fit in a
   * {@code int}, only the low-order 32 bits are returned. Note that this conversion can lose
   * information about the overall magnitude and precision of this {@code Decimal128} value as well
   * as return a result with the opposite sign. Note that {@code #NEGATIVE_ZERO} is converted to
   * {@code 0}.
   *
  ...
  /**
   * Converts this {@code Decimal128} to a {@code long}. This conversion is analogous to the
  ...
  /**
   * Converts this {@code Decimal128} to a {@code float}. This conversion is similar to the
   * <i>narrowing primitive conversion</i> from {@code double} to {@code float} as defined in
   * <cite>The Java&trade; Language Specification</cite>: if this {@code Decimal128} has too great a
   * magnitude to represent as a {@code float}, it will be converted to {@link
   * Float#NEGATIVE_INFINITY} or {@link Float#POSITIVE_INFINITY} as appropriate. Note that even when
   * the return value is finite, this conversion can lose information about the precision of the
   * {@code Decimal128} value. Note that {@code #NEGATIVE_ZERO} is converted to {@code 0.0f}.
   *
  ...
  /**
   * Converts this {@code Decimal128} to a {@code double}. This conversion is similar to the
   * <i>narrowing primitive conversion</i> from {@code double} to {@code float} as defined in
   * <cite>The Java&trade; Language Specification</cite>: if this {@code Decimal128} has too great a
   * magnitude to represent as a {@code double}, it will be converted to {@link
   * Double#NEGATIVE_INFINITY} or {@link Double#POSITIVE_INFINITY} as appropriate. Note that even
   * when the return value is finite, this conversion can lose information about the precision of
   * the {@code Decimal128} value. Note that {@code #NEGATIVE_ZERO} is converted to {@code 0.0d}.
   *
  ...
  /**
   * Returns true if the encoded representation of this instance is the same as the encoded
   * representation of {@code o}.
   *
   * <p>One consequence is that, whereas {@code Double.NaN != Double.NaN}, {@code new
   * Decimal128("NaN").equals(new Decimal128("NaN")} returns true.
   *
   * <p>Another consequence is that, as with BigDecimal, {@code new Decimal128("1.0").equals(new
   * Decimal128("1.00")} returns false, because the precision is not the same and therefore the
   ...
   * @see <a
   *     href="https://github.com/mongodb/specifications/blob/master/source/bson-decimal128/decimal128.rst#to-string-representation">
