...
public class ActiveJBufferDataTest extends BobBsonBufferTest {
  @BeforeEach
  public void setup() {
    data = new byte[1000];
    writeSut = new ActiveJBufferData(data, 0, 0);
    readSut = new ActiveJBufferData(data, 0, 1000);
    comp = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN);
  }

  @Test
  public void testByteBufConstructor() {
    byte[] data = new byte[20];
    ByteBuffer buffer = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN);
    buffer.putInt(14);

    var sut = new ActiveJBufferData(ByteBuf.wrapForReading(data));
